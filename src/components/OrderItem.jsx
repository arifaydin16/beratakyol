    ...
    return (
        <div className="p-2 rounded-4xl bg-[var(--pink-gray)] relative">
            <div className="flex justify-between w-full">
                <div className="flex flex-col w-full items-between">
                    <div className="md:flex gap-4">
                        <div className='max-w-[250px]'>
                                ...
                                <div className="relative  flex">
                                    {
                                        order?.products?.map((product, $index) => (
                                            <div className="w-8 relative h-fit">
                            ...
                            {
                                order?.order_note ?
                                    <div className="text-gray-500 text-sm p-2 pl-0">Sipariş Notu: {order?.order_note}</div>
                                    : ''
                            }
                        </div>
                        <div className={`md:ml-16 md:flex-1 md:mt-0 mt-4 transition-[max-height] duration-500 ease-in-out overflow-hidden ${tabOpen ? 'max-h-[100vh] lg:max-h-[30vh]' : 'max-h-0'} hidden md:flex flex-col gap-2`}>
                            {
                                order?.products?.map((product, $index) => (
                                    <div key={$index} className="flex gap-4 items-center">
                                        <span className="font-normal text-xs md:text-sm">{product?.product_name}</span>
                                        <span className="font-normal text-gray-500 text-xs md:text-sm">Ürün Kodu:</span>
                                        <span className="font-normal p-1 bg-gray-200 rounded text-xs md:text-sm flex items-center gap-1 cursor-pointer dark:text-stone-900" onClick={() => window.navigator.clipboard.writeText(product?.product_code)}>
                            ...
                            <div className="flex gap-2 w-full justify-start items-center mt-4 md:mt-0">
                                <div className="p-1 bg-gray-200 dark:text-gray-900 flex gap-2 items-center text-xs rounded-lg">
                ...
                <div className="p-1 md:p-4 max-md:w-full overflow-hidden w-1/2">
                    <div className="flex items-start justify-end gap-4 mb-2">
                        {
                            tabOpen ?
                                ''
                                :
                                <div className="flex flex-col max-md:hiddens items-center justify-center">
                        ...
                        <div className="flex flex-row max-md:hidden items-end gap-4 justify-end">
                            {
                                order?.order_status === 'DECLINED' ?
                                    <div className="flex flex-col items-center justify-center">
                                        ...
                                        <div className="text-gray-500 text-center text-xs">Desteğe <br /> Bağlan</div>
                                    </div>
                                    : ''
                            }
                        ...
                        <div>
                            <div className="text-xs text-gray-500 text-right md:whitespace-nowrap">{new Date(order?.order_date).toLocaleString('tr-TR')}</div>
                            <div className="font-bold text-xl text-right md:whitespace-nowrap">{Intl.NumberFormat('tr-TR').format(order?.price)} ₺</div>
                        </div>


                        <div onClick={() => setTabOpen(prev => !prev)} className="flex justify-end">
                            <svg className="text-[var(--primary)] duration-200 cursor-pointer" style={tabOpen ? { rotate: '180deg' } : {}} xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24">
                    ...
                    <div className="text-xs text-gray-500 text-right md:hidden md:whitespace-nowrap">{new Date(order?.order_date).toLocaleString('tr-TR')}</div>
                    <div className="font-bold text-xl text-right md:hidden md:whitespace-nowrap">{Intl.NumberFormat('tr-TR').format(order?.price)} ₺</div>
                    <div className="flex flex-col md:hidden items-end gap-4 justify-end mt-4">
                        {
                            order?.order_status === 'DECLINED' ?
                                <div className="flex flex-col items-center justify-center">
                                    <svg className='bg-white text-red-500 p-1 rounded-full' xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" color="currentColor"><path d="M17 11.805c0-.346 0-.519.052-.673c.151-.448.55-.621.95-.803c.448-.205.672-.307.895-.325c.252-.02.505.034.721.155c.286.16.486.466.69.714c.943 1.146 1.415 1.719 1.587 2.35c.14.51.14 1.044 0 1.553c-.251.922-1.046 1.694-1.635 2.41c-.301.365-.452.548-.642.655a1.27 1.27 0 0 1-.721.155c-.223-.018-.447-.12-.896-.325c-.4-.182-.798-.355-.949-.803c-.052-.154-.052-.327-.052-.672zm-10 0c0-.436-.012-.827-.364-1.133c-.128-.111-.298-.188-.637-.343c-.449-.204-.673-.307-.896-.325c-.667-.054-1.026.402-1.41.87c-.944 1.145-1.416 1.718-1.589 2.35a2.94 2.94 0 0 0 0 1.553c.252.921 1.048 1.694 1.636 2.409c.371.45.726.861 1.363.81c.223-.018.447-.12.896-.325c.34-.154.509-.232.637-.343c.352-.306.364-.697.364-1.132z" /><path d="M20 10.5V9c0-3.866-3.582-7-8-7S4 5.134 4 9v1.5m16 7c0 4.5-4 4.5-8 4.5" /></g></svg>
            ...
            <div className={`md:ml-16 md:flex-1 md:mt-0 mt-4 transition-[max-height] duration-500 ease-in-out overflow-hidden ${tabOpen ? 'max-h-[100vh] lg:max-h-[30vh]' : 'max-h-0'} hidden max-md:flex flex-col gap-2`}>
                {
                    order?.products?.map((product, $index) => (
